...
Despite this lack of formal structure, survey results indicate that datapack creators organically replicate some Agile principles. For instance, many rely on iterative release cycles, "fail fast" methodologies, and rapid viability testing via pseudocode.
...
Datapack developers encounter a more extreme version of this phenomenon. Due to the lack of the previously described programming features, writing standard procedural code is not always possible.
Instead, they rely heavily on third-party precompilers, metacompilation, and external scripts to generate the necessary boilerplate. While these tools allow developers to bypass the DSL's assembly-like limitations, they introduce significant maintenance overhead.
...
Future research could employ Mining Software Repositories (MSR) techniques to empirically analyze publicly available datapack codebases. By programmatically evaluating these projects, researchers could quantitatively assess code quality and size, and identify common structural anti-patterns, such as inefficient execution loops or namespace collisions, that qualitative survey data may not fully capture.