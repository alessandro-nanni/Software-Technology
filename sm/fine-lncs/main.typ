...
Viggiato et al. extended this approach to surveying across multiple domains, conducting interviews and web surveys to capture how development practices vary by context @viggiato2019.
...
To efficiently collect this data, McGuire utilized 5-point Likert-scale surveys covering 79 distinct factors across categories like project management, teamwork, change management, and quality focus @mcguire1996. While McGuire noted that Likert scales are inherently limited to capturing subjective perceptions rather than objective facts, they remain a highly effective means of quickly gathering data and revealing overarching trends in developer thinking @mcguire1996. I apply this same principle more narrowly in my primary study, using linear scales on select questions (Q14, Q15, Q16, Q17) to gauge specific process related behaviors, even if their coding environment fundamentally lacks formal structure.
...
When asked about their preferred code editor (Q&), almost 90% of the participants have claimed to use Visual Studio Code with datapack specific extensions#footnote[#link("https://marketplace.visualstudio.com/items?itemName=SPGoding.datapack-language-server")]. This is an expected outcome since it's the tool that most closely resembles an IDE for datapack development, with Q8 highlighting that all of its core features are used.
...
In response to Q15, more than half (55.8%) of the respondants that work in groups do not have a structured development process. No one claimed to use Scrum, and only two respondents use extreme programming.
...
In contrast, standard end user programmers tend to be overconfident in their code and debug opportunistically rather than systematically @ko_state_2011. To support them, researchers advocate for the "What You See Is What You Test" (WYSIWYT) methodology, where users incrementally validate any value as correct at any point during the development process @burnett_2004. Behind the scenes, these validations measure the quality of testing against an adequacy criterion#footnote[A measurable standard, such as the percentage of statements or branches exercised by existing tests, used to decide how much of the program has actually been checked.] and project the results via visual devices to guide the user's focus @burnett_2004. EUSE systems also employ "Help-Me-Test" algorithms, which analyze a program's data dependency chains in the background to generate inputs that increase test coverage, and, when combined with user-defined assertions, shift into an adversarial mode that actively searches for inputs violating those assertions @burnett_2004.
