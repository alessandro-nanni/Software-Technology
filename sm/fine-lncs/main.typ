...
The remainder of this paper is structured as follows. Section 2 provides the theoretical background of the Minecraft DSL, detailing its syntactical limitations and structural constraints. Section 3 reviews related literature in DSL usability, video game development, and end user software engineering. Section 4 presents the survey methodology and descriptive results. In Section 5, I discuss these findings against established academic paradigms, and separately report a quantitative statistical analysis of correlations within the survey data (my research hypotheses). Finally, Section 6 concludes the paper.
...
To contextualize the constraints of the Minecraft _mcfunction_ environment, it is crucial to first understand DSLs in general. A DSL is a programming language tailored to a specific application domain, designed to offer substantial gains in expressiveness and ease of use compared to GPLs @KOSAR201677#footnote[This paper was identified via the systematic literature search and selection protocol described in @lit-selection.]. DSLs are foundational to modern software methodologies like Generative Programming#footnote[An approach that automates the creation of program variants from reusable, configurable components, reducing the effort of writing each similar piece of software by hand.], Software Factories#footnote[An approach that combines DSLs, templates, and design patterns into an assembly line-like process for producing a specific family of software products, analogous to how a physical factory mass-produces configurable goods.], and Model Driven Engineering#footnote[A methodology in which abstract models, rather than hand written code, serve as the primary development artifact, with concrete implementations automatically generated from these models.] @KOSAR201677.

When evaluating a DSL in practice, researchers look at specific success factors: reliability, usability, productivity, learnability, expressiveness, and reusability @hermans2009#footnote[Also identified via the search protocol described in @lit-selection]. For instance, industrial use of DSLs has been shown to successfully automate trivial tasks, significantly reduce time to market, and improve software maintainability @hermans2009. However, systematic mapping studies reveal a gap in the literature: while the DSL community heavily focuses on developing new implementation techniques, there is a distinct lack of rigorous empirical evaluation, validation, and maintenance research @KOSAR201677. By surveying actual datapack developers, my study contributes empirical data toward this gap by documenting how developers experience and work around the DSL's limitations in practice. The Minecraft DSL presents a unique case because it organically evolved from simple commands into a Turing complete language, yet it still forces developers to rely on external workarounds due to its lack of high level constructs and traditional control flow.
...
A *function* is a file with the `.mcfunction` extension, which contains one command per line. When a function is invoked, the commands inside it are run one after another.
...
When mapped to conventional Java development paradigms, this directory hierarchy closely mirrors standard project architectures. Specifically, `my_datapack` functions as the project's root directory, `my_namespace` operates as the Java package, and `my_function.mcfunction` acts as the individual source file.

== Weaknesses of the DSL <ch-weaknesses>

To precisely characterize the weaknesses discussed in this section, the analysis borrows terminology from the Cognitive Dimensions of Notations (CDN) framework, which Albuquerque et al. instantiated into a quantitative metric suite for evaluating DSL usability during software maintenance @ALBUQUERQUE2015#footnote[Likewise identified via the search protocol described in @lit-selection]. The CDN framework is a collection of properties, called cognitive dimensions (CDs), such as viscosity, diffuseness, and hidden dependencies, used to evaluate and compare the usability of any notational system, textual or otherwise, independently of its specific domain.

Because the _mcfunction_ DSL was not initially conceived as a Turing complete language, it still carries some limitations from its original design as a simple way to bypass gameplay obstacles. At first, each command was designed to give users advantages in a specific domain of the game, unrelated to the others. For this reason, to this day, each command is strongly decoupled from the next: there is no proper way to carry context over without relying on an external data source.

This decoupled design philosophy can be seen in the command in charge of math operations, `scoreboard`. Not only is it limited to performing only the 4 basic arithmetic operations on integers, but it also cannot singlehandedly compute operations that involve more than two values. In addition to that, division and multiplication do not natively support constants, so they have to be declared as variables#footnote[By convention, community members typically prefix variable names with `$` and constant names with `#`, as seen in the listing below.] before being used. This is a clear case of high viscosity, the resistance a notation offers to small changes, since altering a single constant requires editing multiple statements spread across the file rather than a single value @ALBUQUERQUE2015. An example that illustrates how `int x = (y*2)/4-2` is calculated in _mcfunction_ can be seen in @scoreboard_set_const.
#figure(
  ```
  # data initalization
...
As previously mentioned, the _mcfunction_ DSL lacks traditional local variables, so developers must utilize external data sources to pass information between commands. These sources can either be tangible, world specific objects like blocks and entities#footnote[Entities refer to any moving or dynamic object not restricted to the block grid (https://minecraft.wiki/w/Entity).], or purely virtual data structures maintained entirely in the game's memory. In either case, the data is structured as Named Binary Tags (NBT#footnote[https://minecraft.wiki/w/NBT_format]). This type of data can be stringified and manually edited by developers as SNBT. Its structure is quite similar to that of a JSON object, but without double quotes surrounding the keys. Because context is carried through blocks, entities, or scoreboards rather than through the command sequence itself, the relations between a command and the state it depends on are not visible in the code. This is a case of high Hidden Dependencies: the extent to which relations between entities are not visible directly in the notation itself @ALBUQUERQUE2015.
...
While the example in @recursion-ex illustrates a basic loop, real world scenarios usually involve additional commands that run before or after the loop and must not be repeated. For this reason, the iterative block must be extracted into a separate function file. This necessity severely complicates the development experience. Given how fundamental iteration is for tasks like raycasting#footnote[A technique that simulates vision or detects obstacles by projecting an invisible line (ray) from a point and checking what it intersects with.], traversing arrays or implementing function approximation algorithms, having to create a new physical file every single time a loop is required adds massive management overhead to the project.
...
Previously namespaces were mentioned as a means to avoid naming collisions. However, creating files (functions) in different namespaces does not prevent conflicts in other aspects of the datapack.
#quote(
  block: true,
  attribution: [Nathan Adams#footnote[A minecraft developer, part of the team that develops datapack related features.]],
...
In conclusion, simply working in a separate folder is not enough to guarantee compatibility with other datapacks.

= Related Works

== Selection and Overview of Reference Literature <lit-selection>

Due to the complete absence of prior scientific literature focusing specifically on Minecraft datapack development, reference papers were systematically selected from broader software engineering research areas. Rather than conducting a full systematic literature review, I adapted the search string construction and inclusion/exclusion procedures from Kitchenham and Charters' SLR protocol @kitchenham2007 to keep the paper selection process reproducible and unbiased. A search was conducted across digital databases (including Google Scholar, the ACM Digital Library, IEEE Xplore, ScienceDirect, and SpringerLink) using combinations of search terms such as: "DSL usability evaluation", "indie game QA practices", "end user programming", "hobbyist developer productivity", and terms targeting empirical "surveys" or "primary studies".
...
- *EC1*: Studies focusing solely on compiler construction, formal grammar theory, or programming language specifications were excluded, since these highly technical implementation studies lack adaptable empirical methods or process metrics. As highlighted by a systematic mapping study of the DSL domain, the vast majority of DSL literature (79.3%) focuses on developing new techniques and methods, with only 3.3% dedicated to measuring the effectiveness of DSL approaches @KOSAR201677.

Applying these criteria yielded a final group of thirteen core papers, structured around three thematic groups to establish my methodological framework:
...
2. *Video Game Development and Quality Assurance*: Datapack development is inherently a creative endeavor with practices akin to game development. Thus, I selected research focusing on indie and unconventional software development environments, which is characterized by unstructured processes and improvised testing. Musil et al. @musil2010 provides a baseline for evaluating the state of practice in video games; Cho et al. @cho2023 offers validated methodologies for studying quality assurance in indie/hobbyist contexts; and Viggiato et al. @viggiato2019 helps analyze how development and integration practices vary across creative domains. Beyond their thematic content, Musil et al. and Cho et al. also document their own survey creation process, respectively a GQM structured design and a domain expert piloting phase, which directly influenced how I constructed and validated my own questionnaire, as detailed in @survey and @survey-process.
3. *End User Software Engineering (EUSE) and Project Management*: Since datapack development is undertaken without corporate contracts or formal structures, I must model developers' work using paradigms tailored to informal environments, consistent with EUSE's focus on personal rather than commercial purpose. I selected studies on how non professional programmers build and debug software (Ko et al. @ko_state_2011, Burnett et al. @burnett_2004), how Agile and Continuous Integration (CI) scale down to small or hobbyist teams (Noteboom et al. @noteboom_agile_2021, Chen et al. @chen_continuous_2025, Banker et al. @banker_software_1998), and how metrics apply to corporate versus open source environments (McGuire @mcguire1996, Canedo et al. @canedo2019).
...
Finally, to ensure the survey's effectiveness, I adopted the same validation phase used by Cho et al. @cho2023, piloting the questionnaire with domain experts to refine its flow and phrasing before wide deployment.

== Empirical Surveys in Software Houses

Datapack creation shares many characteristics with independent, unstructured game development, but my study's goals also extend to assessing whether datapack developers exhibit formal project management and software engineering practices. To build this comparison, I examined how empirical surveys are conducted within software houses, where structured development processes are the norm.
...
To efficiently collect this data, McGuire utilized 5-point Likert-scale surveys covering 79 distinct factors across categories like project management, teamwork, change management, and quality focus @mcguire1996. While McGuire noted that Likert scales are inherently limited to capturing subjective perceptions rather than objective facts, they remain a highly effective means of quickly gathering data and revealing overarching trends in developer thinking @mcguire1996. I apply this same principle more narrowly in my primary study, using linear scales on select questions to gauge specific process related behaviors, even if the datapack developer coding environment fundamentally lacks formal structure.
...
== My Proposed Survey Preparation Process <survey-process>
...
/ Phase 2: Adapting DSL and Game Dev Metrics. To capture the unique constraints of the `mcfunction` DSL, I incorporated a metric based on a known DSL success factor: the usability of external tooling (Q8) @hermans2009. Albuquerque et al. group their usability metrics into two overarching characteristics, expressiveness and conciseness, each built from individual CDs such as viscosity and diffuseness @ALBUQUERQUE2015. Rather than measuring these through survey questions, I address them analytically in @ch-weaknesses using the CDN framework, and they also surface organically in the open ended responses (Q23-Q24). Because datapack creation shares similarities with the chaotic, unstructured testing approaches commonly seen in independent game development @cho2023, relying solely on predefined checkboxes and linear scales is insufficient. To capture the highly unconventional and improvised practices of these creative environments, the survey incorporates an "Other" field into these questions. This ensures that respondents have the option to write in a custom response if their specific methods are not available in the predefined choices.
/ Phase 3: Testing and Piloting. Before making the survey public, a small group of domain experts piloted the questionnaire, following the same validation practice used to ensure clarity in empirical survey design @cho2023. Their feedback confirmed that questions regarding tools and standard practices were interpreted correctly, and helped make the wording clearer for a broader audience.
/ Phase 4: Execution and Community Targeting. I distributed the survey within specialized online datapack communities. While this introduces a selection bias toward more "hardcore" or professional hobbyists, it ensures high external validity, providing me with meaningful answers from dedicated developers who actively interact with the DSL's limitations and workarounds.
...
Following the structured methodologies seen in related DSL and software engineering studies, this section is dedicated to presenting the quantitative and qualitative data obtained from the respondents. Therefore, I will not be comparing these results against other domains here. Instead, I discuss the most salient of these findings against established literature on professional software engineering practices and end user software engineering (EUSE) paradigms in the subsequent @discussion_ch.

It is immediately noticeable how most of the respondents consider themselves very experienced in both datapack and broader computer programming (Q2 & Q4). As shown in @exp-distr, 82.5% of respondents rated their datapack experience at 5 or higher on the 7-point scale, and 69.1% did the same for their general programming familiarity, with both distributions having a median of 6. This helps guarantee the overall validity of the survey, since the answers provided will be backed up by concrete experience within the domain of the Minecraft DSL as well as other GPLs.

#figure(
  image("process/experience_distribution.png"),
  caption: [Self-reported experience levels of survey respondents (N=97). (A) Distribution of ratings for Q2 (datapack development experience) and Q4 (general programming familiarity). (B) Spread of ratings for both questions, with the "very experienced" threshold (5+) marked.],
) <exp-distr>
...
When asked about their preferred code editor (Q7), almost 90% of the participants have claimed to use Visual Studio Code with datapack specific extensions#footnote[#link("https://marketplace.visualstudio.com/items?itemName=SPGoding.datapack-language-server")]. This is an expected outcome since it is the tool that most closely resembles an IDE for datapack development, with Q8 highlighting that all of its core features are used.
...
When asked which software development process they use when working in a group (Q15), more than half (55.8%) of the respondents that work in groups reported not having a structured development process. No one claimed to use Scrum, and only two respondents used extreme programming.

With regard to coding conventions (Q21), 81.4% of the respondents claimed to adopt different namespaces across projects, and 48.5% also used the more advanced _Smithed_ conventions#footnote[Smithed is a community that defined a set of naming and structural conventions, aswell as developing a "standard library" ecosystem for datapack development (https://docs.smithed.dev/index.html).]. Six users stated that they do not know what coding conventions are or that they do not use them. Two responses also mentioned the use of IMP-doc, a comment convention similar to Javadoc, but applied to datapack functions.

When asked about their habits for bug tracking and feature requests (Q22), two out of three respondents claimed to use informal methods such as Discord channels, and 52.6% used GitHub issues.
...
#figure(
  table(
    ...
    table.hline(),
    [1], [Uses fail fast methodology#footnote[The fail fast methodology is a philosophy focused on rapid experimentation to minimize wasted time, effort, and resources. It operates on the premise that discovering a flaw or an unviable idea as early as possible is a successful outcome.]],
    table.hline(),
...
#figure(
  table(
    ...
    table.hline(),
    [2], [Sometimes it is more efficient to hardcode function outputs rather than calculating them at runtime.],
    table.hline(),
...
Based on the reported techniques and frustrations with the system, the typical datapack creator can be profiled as a highly technical, modern software engineer engaging in a complex, problem solving hobby. This individual possesses an advanced understanding of general programming paradigms but is paradoxically drawn to the restrictive, assembly language environment of the Minecraft DSL due to the intellectual challenge it presents. Rather than accepting the language's severe limitations#footnote[The respondents identified the same limitations discussed earlier in Section 2.3: the absence of native control flow, complex mathematics, but also highlighted the inability to perform string manipulation.], they engineer sophisticated workarounds by building custom abstractions and utilizing external precompilers. They approach this video game modification platform with an almost professional rigor, organically adapting informal agile principles, writing pseudocode, and rejecting artificial intelligence in favor of reliable, accurate third party tools. However, this ambition is frequently coupled with persistent frustration regarding software maintenance, as they constantly battle structural disorganization, cross version or backward incompatibility, and the need to write repetitive boilerplate code.
...
#figure(
  ...
  caption: [Average response to questions that use a linear scale. It is evident how users under the age of 18 are more reliant on external libraries and LLMs. They also indicated their experience in computer programming and datapack development to be less, compared to the other age groups. They are also the ones that comment code the least, probably because they are unaware of the benefits of commenting in the long term. It is also interesting to notice how the younger group is the one that develops datapacks for compensation the most. This perhaps could be related to the fact that they are looking for a small source of income, whereas older developers with a steady source of income develop datapacks purely as a hobby.],
) <avg-responses>
...
== Threats to Validity
Internal validity concerns the structure of the survey and the reliability of the collected responses. External validity addresses the generalizability of these findings to the broader developer community and their longevity over time.

=== Internal Validity
- Insufficient knowledge for young developers. 13.4% of the respondents are either 18 years or younger. It is very unlikely for people at that age to be familiar with software development or project management concepts, which are usually learned in higher education or through work experience.
...
- In Q23, three participants commented that they would have liked more questions to be checkboxes rather than multiple choice, since not always one option excluded the others. However, this does not mean that the data gathered for these users is wrong or unusable, just incomplete.
- Social Desirability Bias: Developers know they should comment their code, use Git, and adhere to coding conventions#footnote[Additionally, the use of LLMs to generate code is seen as bad practice since it does not produce valid syntax and takes the fun out of what is generally seen as a passion project.]. Therefore, it is likely that Q11, Q13, Q16 and Q21 are skewed towards the "good" choices since the participants want to appear professional.

=== External Validity
- The survey was undertaken by members of online communities specialized in the development of datapacks. This made the risk of getting empty or incomplete responses significantly low, with only one participant commenting that they felt like an outlier while answering.
...
Datapack developers encounter a more extreme version of this phenomenon. Due to the lack of the previously described programming features, writing standard procedural code is not always possible. Instead, they rely heavily on third party precompilers, metacompilation, and external scripts to generate the necessary boilerplate. While these tools allow developers to work around these constraints, they introduce significant maintenance overhead.
...
In contrast, standard end user programmers tend to be overconfident in their code and debug opportunistically rather than systematically @ko_state_2011. To address this, researchers advocate for the "What You See Is What You Test" (WYSIWYT) methodology, where users incrementally validate any value as correct at any point during the development process @burnett_2004. Behind the scenes, these validations measure the quality of testing against an adequacy criterion#footnote[A measurable standard, such as the percentage of statements or branches exercised by existing tests, used to decide how much of the program has actually been checked.] and project the results via visual devices to guide the user's focus @burnett_2004. EUSE systems also employ "Help-Me-Test" algorithms, which analyze a program's data dependencies in the background to generate inputs that increase test coverage. If the user has defined assertions, the algorithm instead searches for inputs that violate them @burnett_2004.

Datapack developers occupy a middle ground between these two extremes. While they exhibit high technical proficiency, with a vast majority utilizing formal version control systems, only a small fraction (around 25%) implement formal CI/CD pipelines. Because establishing robust CI infrastructure requires high initial costs and architectural planning @chen_continuous_2025, datapack quality assurance processes instead depend heavily on manual beta testing and informal community feedback. They do not have access to automated fault localization capabilities that visually prioritize which suspicious code snippet to investigate first @burnett_2004. Furthermore, the Minecraft environment lacks both an equivalent to "Help-Me-Test" algorithms, which would analyze command structures (such as `execute if` conditions or `scoreboard` dependencies) to automatically generate coverage increasing test values, and any native assertions mechanism for such an algorithm to pair with in actively searching for fault revealing inputs. As a result, developers must manually open the game, set up complex entity states or scoreboards, and verify every edge case individually.

This low CI/CD adoption rate should not be read purely as a symptom of insufficient resources or technical maturity. Viggiato et al. found that CI is not adopted as a rigid, universal practice even within professional, well resourced organizations: developers in the banking and e-commerce domains deliberately freeze new deployments through their CI/CD pipelines during critical commerce periods, such as Black Friday, the weeks before the new year, and salary payment cycles. This freeze avoids the risk that even a fully tested change could still introduce an unforeseen bug during the exact window when system reliability matters most @viggiato2019. This validated, cross domain finding shows that even mature corporate environments calibrate CI adoption against real external risk rather than applying it uniformly. More broadly, the same study found that a company's policy and culture, rather than the intrinsic technical demands of a domain, are what most strongly shape which development practices get adopted and how @viggiato2019. Applying this lens to datapack development, the community's informal and inconsistent use of CI/CD can be read less as a maturity deficit and more as a rational adaptation: lacking both a company level policy structure and any external commercial risk to guard against, datapack developers have little structural pressure to adopt the standardized pipelines seen in industry.

Since hobbyists cannot be expected to build or adopt a dedicated testing practice on their own initiative, future tooling aiming to improve QA habits in this community should favor passive, unobtrusive cues over a standalone testing framework, following the "Surprise-Explain-Reward" strategy: a cognitive tutoring approach designed to encourage end user programmers (who are usually focused on getting their code to work rather than writing clean tests) to adopt good software engineering habits @burnett_2004. An example can be seen in editors that underline code detected as deprecated, improvable or unsafe#footnote[This can partly be seen in the Spyglass VSCode extension when using expensive selectors. Warnings are raised when inefficient, broad selections are used.], nudging developers toward better practices without requiring them to seek anything out.

== Quantitative Analysis of Specific Survey Hypotheses

In addition to the qualitative comparison above, this section presents a separate statistical analysis of seven research hypotheses concerning developer demographics, development practices, code abstractions, team collaboration, and QA efforts.
...
- *Descriptive Averages and Variation*: To describe central tendencies, I report the mean ($M$, the arithmetic average) and the median ($"Mdn"$, the middle value of the ordered dataset). To measure the spread of responses, I report the standard deviation ($"SD"$), which quantifies how much individual data points vary from the mean.
...
Additionally, @h1-techniques compares the adoption of software engineering techniques across these groups. Version control usage increases with age, ranging from 76.9% in G1 to 100.0% in G3. Coding convention adoption, by contrast, is uniformly high across all groups (97.6%-100%), showing no meaningful age gap. Precompiler and external script usage is similarly comparable across groups (47.6%-53.8%), with no clear age trend. The clearest divide appears in commenting frequency: only 30.8% of G1 comment frequently, compared to 52.4% in both G2 and G3. This suggests that technical knowledge acquired in university or professional settings (G2/G3) correlates with more consistent commenting and version control habits, though not with markedly different adoption of coding conventions or precompilers, both of which are already comparably widespread across all age groups.
...
#figure(
  ...
  caption: [Scatterplot of refactoring effort vs. release cycle frequency for G2 & G3.],
) <h5-refactor-cycle>

=== Experience and Levels of Abstraction (Hypothesis 6)
I examined the relationship between experience and the level of code abstraction (Plain mcfunction #sym.arrow Custom scripts #sym.arrow Precompilers). As illustrated in @h6-experience-abstraction, experience correlates positively with abstraction level (Spearman's $rho = 0.400$, $p < 0.001$). The median experience rises from $"Mdn" = 5.0$ for plain mcfunction users to $"Mdn" = 7.0$ for precompiler users, showing that advanced workarounds are primarily adopted by more experienced creators.
...
== Summary
Ultimately, the datapack community demonstrates a unique blend of practices. Rather than adopting simplified copies of professional GPL methodologies, they confront the informal challenges arising from the DSL's limitations by developing their own adapted equivalents: version control, code generation workarounds, loosely agile feedback loops, and ad hoc quality assurance. This informal engineering rigor organically emerges and persists even in an entirely decentralized, hobbyist domain, though unevenly and not without friction, as reflected in the inconsistent adoption of structured team processes and the added maintenance overhead introduced by code generators.

= Conclusion
This study investigated the software engineering practices and project management methodologies adopted by the Minecraft datapack community, framed around four goals: characterizing developer demographics, tooling, project management, and quality assurance practices.
Despite operating within a DSL characterized by severe structural limitations, datapack developers exhibit a remarkably high degree of technical proficiency, organically compensating for the engine's constraints through informal, professionally inspired practices, as detailed in Section 5.
Ultimately, this research highlights a unique intersection between End User Software Engineering and professional development paradigms: software engineering rigor is not confined to corporate, well resourced environments, but can emerge organically wherever a community faces severe enough constraints and cares enough about the outcome.

While this primary study establishes a baseline of developer behavior based on self reported data, it remains subject to the same social desirability bias discussed under Threats to Validity, and a significant gap regarding objective code analysis persists.
Future research could employ Mining Software Repositories (MSR) techniques to empirically analyze publicly available datapack codebases directly, avoiding the risk of bias inherent to self reported data. By analyzing the code itself, researchers can measure its size and quality, and spot common coding mistakes (e.g. slow loops, missing namespaces...) that survey answers might miss.
